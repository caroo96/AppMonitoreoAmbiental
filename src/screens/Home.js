...
export default function Home() {
  ...
  const loadSavedData = async () => {
    try {
      ...
      // Primero probar sin filtro de ciudad
      console.log('🔍 Probando carga sin filtro de ciudad...');
      const todosLosDatos = await WeatherAPI.getSavedHistoricalData(null, 50);
      console.log('📊 Todos los datos en Firebase:', todosLosDatos.length, 'registros');
      
      // Luego filtrar por ciudad
      const datos = await WeatherAPI.getSavedHistoricalData(selectedCity, 20);
      console.log('📊 Datos filtrados para', selectedCity, ':', datos.length, 'registros');
      
      setSavedData(datos);
      
      if (datos.length === 0) {
        if (todosLosDatos.length === 0) {
          Alert.alert('Info', 'No hay datos guardados en Firebase. Intenta guardar algunos datos primero.');
        } else {
          Alert.alert('Info', `No hay datos guardados para "${selectedCity}". Hay ${todosLosDatos.length} registros de otras ciudades.`);
        }
      } else {
